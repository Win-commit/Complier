...
void newtable() {
    ...
    // 添加read和write函数
    pItem readFun = newItem(newFieldList("read",newType(FUNCTION, 0, NULL, newType(BASIC, INT_TYPE))));

    pItem writeFun = newItem(newFieldList("write",newType(FUNCTION, 1,newFieldList("arg1", newType(BASIC, INT_TYPE)),newType(BASIC, INT_TYPE))));

    addTableItem(readFun);
    addTableItem(writeFun);
}
